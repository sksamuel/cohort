 ...
 *
 * To configure a check for the file store for the root directory, use [DiskSpaceCheck.root].
 *
...
class DiskSpaceCheck(
  ...
  companion object {
    ...
    /**
     * Returns a [DiskSpaceCheck] for the root file store on the default file system.
     * If there is more than one root directory available, an error will be thrown.
     */
    fun root(minFreeSpacePercentage: Double = 10.0): DiskSpaceCheck {
      val root = FileSystems.getDefault().rootDirectories.single()
      return DiskSpaceCheck(Files.getFileStore(root), minFreeSpacePercentage)
    }

    /**
     * Returns a [DiskSpaceCheck] for each filestore in the get file system.
     */
    fun defaults(minFreeSpacePercentage: Double = 10.0): List<Check> =
      FileSystems.getDefault().fileStores.map { DiskSpaceCheck(it, minFreeSpacePercentage) }
  }
}
